...
void handle_touch_event(int screen_x, int screen_y, bool touch_down,
                        int *touch_State, char *touch_Val,
...
void draw_alert_box(const char *message, int x, int y, int w, int h,
  ...
  draw_text(message, x + w / 8, y + h / 3, 3, text_color, vip, fip, map);
...
// 사용자 인증
int check_balance(const char *userID, const char *bank_account_ID,
                  struct fb_var_screeninfo *vip, struct fb_fix_screeninfo *fip,
                  char *map) {
  ...
  if (row) {
    // DB에서 가져온 잔액을 정수형으로 변환
    int balance = atoi(row[0]);
    printf("Balance: %d\n", balance);
    // 잔액 값을 문자열로 변환하여 메시지 준비 (예: "Balance: 12345")
    char message[64];
    snprintf(message, sizeof(message), "Balance: %d", balance);

    // 잔액이 조회되었으므로, alert 박스처럼 잔액을 표시
    // 여기서는 예시로 배경색은 COLOR_GREEN, 글자색은 COLOR_WHITE를
    // 사용했습니다.
    ...
    // 조회 실패 시(예: 해당 계좌가 없거나 에러 발생)
    mysql_free_result(result);
    db_disconnect(conn);
    draw_alert_box("CANT CHECK BALANCE", 200, 200, 300, 100, COLOR_RED,
...
void handle_touch_event(int screen_x, int screen_y, bool touch_down,
  ...
  draw_text(touch_Val, 25, 318, 5, COLOR_WHITE, &vinfo, &finfo, map);
    ...
    if (ev.type == EV_ABS) {
      ...
      if (ev.code == BTN_TOUCH && ev.value == 0) {  // 터치가 떼어졌을 때
        ...
        handle_touch_event(screen_x, screen_y, ev.code == BTN_TOUCH,
                           &touch_StatePW, touch_ValPW, &cursor_PW);

        if (exit_State == CHECK) {
          // 여기서 db의 아이디와 비밀번호가 일치하는지 체크 한다.

          if (check_balance(touch_ValID, touch_ValPW, &vinfo, &finfo, map)) {
            exit_State = OK;
...
int main(int argc, char *argv[]) {
  ...
  draw_text("ID", 30, 160, 4, COLOR_BLACK, &vinfo, &finfo, map);
  draw_text(user_ID, 25, 208, 4, COLOR_WHITE, &vinfo, &finfo, map);

  draw_rounded_rect(20, 310, 440, 50, 15, COLOR_MAIN, &vinfo, &finfo, map);
  draw_text("CHECK BALANCE", 30, 270, 4, COLOR_BLACK, &vinfo, &finfo, map);
