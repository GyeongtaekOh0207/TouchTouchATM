...
int withdraw_user_account(int money, const char *userID,
                          const char *bank_account,
                          struct fb_var_screeninfo *vip,
                          struct fb_fix_screeninfo *fip, char *map) {
  if (money <= 0) {
    fprintf(stderr, "Invalid withdrawal amount: must be positive.\n");
    return 0;
  }
  ...
  // UPDATE 쿼리: userID와 bank_account에 해당하는 계좌에서 money 만큼 인출
  char query[256];
  snprintf(query, sizeof(query),
           ...
           "AND b.money >= %d;",
           money, userID, bank_account, money);
  ...
  if (affected_rows > 0) {
    // 업데이트가 성공하면, 같은 연결을 이용해 SELECT 쿼리로 잔액을 조회합니다.
    ...
    // 잔액을 문자열로 만들어 alert 박스에 표시 (예: "CURRENT: 12345")
    char message[64];
    ...
    // 인출 금액이 부족하거나 조건에 맞지 않으면 해당 메시지를 표시합니다.
    draw_alert_box("CANT WITHDRAW", 250, 200, 300, 100, COLOR_RED, COLOR_WHITE,
                   vip, fip, map);
    db_disconnect(conn);
    return 0;
  }
...
void *thread_func2(void *arg) {
  ...
  char touch_ValACCNUM[10] = "";
  char touch_ValSUM[10] = "";
    ...
    if (ev.type == EV_ABS) {
      ...
      if (ev.code == BTN_TOUCH && ev.value == 0) {  // 터치가 떼어졌을 때
        ...
        if (exit_State == CHECK) {
          ...
          if (withdraw_user_account(val_sum, touch_ValID, touch_ValACCNUM,
                                    &vinfo, &finfo, map)) {
            exit_State = OK;
...
int main(int argc, char *argv[]) {
  ...
  // deposit amount
  draw_rounded_rect(20, 310, 440, 50, 15, COLOR_MAIN, &vinfo, &finfo, map);
  draw_text("WITHDRAW AMOUNT", 30, 270, 4, COLOR_BLACK, &vinfo, &finfo, map);
