...
struct cursor cursor_WACC;
struct cursor cursor_DACC;
struct cursor cursor_AMOUNT;
...
int transfer_user_account(int money, const char *from_account,
                          const char *to_account, struct fb_var_screeninfo *vip,
                          struct fb_fix_screeninfo *fip, char *map) {
  if (money <= 0) {
    fprintf(stderr, "Invalid transfer amount: must be positive.\n");
    return 0;
  }
  ...
  // 출금 (from_account에서 money 차감)
  snprintf(query, sizeof(query),
           "UPDATE tbl_name a JOIN tbl_bank_account b ON a.ID = b.ID "
           "SET b.money = b.money - %d "
           "WHERE b.bank_account_ID = '%s' AND b.money >= %d;",
           money, from_account, money);

  printf("Executing Withdraw Query: %s\n", query);

  if (mysql_query(conn, query)) {
    fprintf(stderr, "Withdraw Query Failed: %s\n", mysql_error(conn));
    mysql_query(conn, "ROLLBACK;");
    db_disconnect(conn);
    return 0;
  }

  int withdraw_rows = mysql_affected_rows(conn);
  printf("Withdraw Affected Rows: %d\n", withdraw_rows);

  // 출금 실패 시 롤백
  if (withdraw_rows <= 0) {
    fprintf(stderr, "Insufficient balance or invalid account.\n");
    mysql_query(conn, "ROLLBACK;");
    db_disconnect(conn);
    draw_alert_box("CANT WITHDRAW", 250, 200, 300, 100, COLOR_RED,
                   COLOR_WHITE, vip, fip, map);
    return 0;
  }

  // 입금 (to_account에 money 추가)
  snprintf(query, sizeof(query),
           "UPDATE tbl_name a JOIN tbl_bank_account b ON a.ID = b.ID "
           "SET b.money = b.money + %d "
           "WHERE b.bank_account_ID = '%s';",
           money, to_account);

  printf("Executing Deposit Query: %s\n", query);
  ...
  int deposit_rows = mysql_affected_rows(conn);
  printf("Deposit Affected Rows: %d\n", deposit_rows);

  // 입금 실패 시 롤백
  if (deposit_rows <= 0) {
    fprintf(stderr, "Deposit failed: Invalid recipient account.\n");
    mysql_query(conn, "ROLLBACK;");
    db_disconnect(conn);
    draw_alert_box("TRANSFER FAILED", 250, 200, 300, 100, COLOR_RED,
                   COLOR_WHITE, vip, fip, map);
    return 0;
  }
...
void handle_touch_event(int screen_x, int screen_y, bool touch_down,
  ...
  if (*AS_State == WACC) {
    draw_rounded_rect(20, 165, 440, 50, 15, COLOR_MAIN, &vinfo, &finfo, map);
    draw_text(touch_Val, 27, 173, 5, COLOR_WHITE, &vinfo, &finfo, map);
  } else if (*AS_State == DACC) {
    draw_rounded_rect(20, 250, 440, 50, 15, COLOR_MAIN, &vinfo, &finfo, map);
    draw_text(touch_Val, 27, 258, 5, COLOR_WHITE, &vinfo, &finfo, map);
  } else if (*AS_State == AMOUNT) {
    draw_rounded_rect(20, 340, 440, 50, 15, COLOR_MAIN, &vinfo, &finfo, map);
    draw_text(touch_Val, 27, 348, 5, COLOR_WHITE, &vinfo, &finfo, map);
  }

  if (screen_x >= 25 && screen_x <= 400 && screen_y >= 165 && screen_y <= 235) {
    printf("Button WACC pressed\n");
    *AS_State = WACC;
  } else if (screen_x >= 25 && screen_x <= 400 && screen_y >= 250 &&
             screen_y <= 320) {
    printf("Button DACC pressed\n");
    *AS_State = DACC;
  } else if (screen_x >= 25 && screen_x <= 400 && screen_y >= 340 &&
             screen_y <= 410) {
    printf("Button AMOUNT pressed\n");
    *AS_State = AMOUNT;
  }
...
void *thread_func1(void *arg) {
  for (;;) {
    if (AS_State == WACC) {
      draw_letter('I', cursor_WACC.x, cursor_WACC.y, 5, COLOR_WHITE, &vinfo,
                  &finfo, map);
      sleep(1);
      draw_letter('I', cursor_WACC.x, cursor_WACC.y, 5, COLOR_MAIN, &vinfo,
                  &finfo, map);
      sleep(1);
    } else if (AS_State == DACC) {
      draw_letter('I', cursor_DACC.x, cursor_DACC.y, 5, COLOR_WHITE, &vinfo,
                  &finfo, map);
      sleep(1);
      draw_letter('I', cursor_DACC.x, cursor_DACC.y, 5, COLOR_MAIN, &vinfo,
                  &finfo, map);
      sleep(1);
    } else if (AS_State == AMOUNT) {
      draw_letter('I', cursor_AMOUNT.x, cursor_AMOUNT.y, 5, COLOR_WHITE, &vinfo,
                  &finfo, map);
      sleep(1);
      draw_letter('I', cursor_AMOUNT.x, cursor_AMOUNT.y, 5, COLOR_MAIN, &vinfo,
                  &finfo, map);
      sleep(1);
    }
...
void *thread_func2(void *arg) {
  ...
  int touch_StateWACC = 0;
  int touch_StateDACC = 0;
  int touch_StateAMOUNT = 0;
  char touch_ValWACC[15] = "";
  char touch_ValDACC[15] = "";
  char touch_ValAMOUNT[15] = "";
      ...
      if (ev.code == BTN_TOUCH && ev.value == 0) {  // 터치가 떼어졌을 때
        ...
        if (AS_State == WACC) {
          handle_touch_event(screen_x, screen_y, ev.code == BTN_TOUCH,
                             &touch_StateWACC, touch_ValWACC, &AS_State,
                             &cursor_WACC);
        } else if (AS_State == DACC) {
          handle_touch_event(screen_x, screen_y, ev.code == BTN_TOUCH,
                             &touch_StateDACC, touch_ValDACC, &AS_State,
                             &cursor_DACC);
        } else if (AS_State == AMOUNT) {
          handle_touch_event(screen_x, screen_y, ev.code == BTN_TOUCH,
                             &touch_StateAMOUNT, touch_ValAMOUNT, &AS_State,
                             &cursor_AMOUNT);
        }

        if (exit_State == CHECK) {
          // 여기서 db의 계좌번호와 잔금내역이이 일치하는지 체크 한다.
          int val_sum = atoi(touch_ValAMOUNT);

          if (transfer_user_account(val_sum, touch_ValWACC, touch_ValDACC,
                                    &vinfo, &finfo, map)) {
            exit_State = OK;
...
int main(int argc, char *argv[]) {
  ...
  // account account
  draw_rounded_rect(20, 165, 440, 50, 15, COLOR_MAIN, &vinfo, &finfo, map);
  draw_text("WITHDRAW ACCOUNT", 30, 140, 3, COLOR_BLACK, &vinfo, &finfo, map);

  // deposit account
  draw_rounded_rect(20, 250, 440, 50, 15, COLOR_MAIN, &vinfo, &finfo, map);
  draw_text("DEPOSIT ACCOUNT", 30, 225, 3, COLOR_BLACK, &vinfo, &finfo, map);

  // account number
  draw_rounded_rect(20, 340, 440, 50, 15, COLOR_MAIN, &vinfo, &finfo, map);
  draw_text("AMOUNT", 30, 315, 3, COLOR_BLACK, &vinfo, &finfo, map);
